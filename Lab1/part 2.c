...
int main(int argc, char **argv) {
    ...
    long n = 10000000; // fixed input size

    if (size < 64) {
        if (rank == 0) {
            fprintf(stderr, "Error: need at least 64 ranks (got %d)\n", size);
        }
        MPI_Finalize();
        return 1;
    }
    ...
    // Each rank picks its own threshold S
    int S = rank + 1;
    comparisons = 0;
    ...
    if (rank == 0) {
        printf("S,Comparisons,Time(s)\n");
        for (int i = 0; i < size; i++) {
            printf("%d,%lld,%.6f\n", i + 1, all_comparisons[i], all_times[i]);
        }
